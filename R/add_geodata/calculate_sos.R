...
Data<-miceadds::load.Rdata2("POWER.CHIRPS.RData","Large Files/")

CLIM<-Data[,list(Site.Key,Date,Rain,ETo)
...
CLIM.MONTH<-CLIM[,list(Rain.M.Sum=sum(Rain)),by=list(Site.Key,Year,Month) # Sum rainfall by month within year and site
...
CLIM<-merge(CLIM,CLIM.MONTH[,list(Site.Key,Month,Rain.Season)],by=c("Site.Key","Month"))
# Ensure dataset is correctly ordered
CLIM<-CLIM[order(Site.Key,Year,Dekad)]
# 3) Classify rainy seasons ####
...
CLIM.Dekad<-CLIM[,list(Rain.Dekad=sum(Rain),AI=mean(AI)),by=list(Site.Key,Year,Dekad,Rain.Season) # Sum rainfall and take mean aridity Site.Key by dekad  (within year, site and rain season)
...
# 3) Calculate Seasonal Values ####
Len<-CLIM.Dekad[,length(unique(Year))]
Seasonal<-unique(CLIM.Dekad[!(is.na(Dekad.Season)|is.na(Start.Year)),list(Site.Key,Start.Year,SOS,EOS,LGP,Dekad.Season,Tot.Rain)])
...
# 4) Roll back SOS where SOS is fixed ####
...
# 4.1) Scenario 1: SOS fixed and one season present #####
...
if(length(Sites)>0){
  ...
  CLIM.Dekad2<-CLIM[Site.Key %in% Sites,list(Rain.Dekad=sum(Rain),AI=mean(AI)),by=list(Site.Key,Year,Dekad,Rain.Season) # Sum rainfall and take mean aridity Site.Key by dekad  (within year, site and rain season)
  ...
  CLIM.Dekad<-rbind(CLIM.Dekad[!Site.Key %in% Sites],CLIM.Dekad2)
}

# 4.2) Scenario 2: SOS fixed and two seasons present #####

# Subset data
Data<-CLIM.Dekad[Site.Key %in% X[Seasons==2,Site.Key]]
...
if(length(Sites)>0){
  ...
  CLIM.Dekad1<-CLIM[Site.Key %in% Sites,list(Rain.Dekad=sum(Rain),AI=mean(AI)),by=list(Site.Key,Year,Dekad,Rain.Season)
  ...
  CLIM.Dekad<-rbind(CLIM.Dekad[!Site.Key %in% c(Sites)],CLIM.Dekad1)
  Clim.Dekad1<-NULL
}


# 4.2.1) Seasons are not adjacent ######
...
if(length(Sites)>0){
  ...
  CLIM.Dekad1<-CLIM.Dekad1[order(Site.Key,Year,Dekad)]
  ...
  CLIM.Dekad1<-CLIM.Dekad1[,Dekad.Seq:=SOS_UniqueSeq(Dekad.Season),by=Site.Key # Sequences within sites need a unique ID
  ][,Complete:=length(Dekad)==36,by=list(Site.Key,Year) # Calculate dekads within a year
  ][Complete==T | (Dekad %in% 34:36 & Year==min(Year)) # Remove incomplete years but keep last three dekads (when wet period start is Jan we need to look 3 dekads before this) 
  ][,Complete:=NULL # Tidy up
  ][,Rain.sum2:=round(c(zoo::rollsum(Rain.Dekad,3),NA,NA)-Rain.Dekad,3) # Rainfall for next two dekads
  ...
  CLIM.Dekad<-rbind(CLIM.Dekad[!Site.Key %in% Sites],CLIM.Dekad1)
  Clim.Dekad1<-NULL
}

# 4.3) Recalculate seasonal values #####

Seasonal2<-unique(CLIM.Dekad[!(is.na(Dekad.Season)|is.na(Start.Year)),list(Site.Key,Start.Year,SOS,EOS,LGP,Dekad.Season,Tot.Rain)])
# Remove second seasons that are too short
...
# 4.4) Add separation #####
CLIM.Dekad[!(is.na(EOS)|is.na(SOS)|is.na(Dekad.Season)),Season.Sep.Min:=SeasonSpacing(SOS,EOS,Dekad.Season)$sepmin,by=Site.Key
...
CLIM.Dekad1<-data.table::copy(CLIM.Dekad)[Site.Key %in% Sites
...
Seasonal3<-unique(CLIM.Dekad1[!(is.na(Dekad.Season)|is.na(Start.Year)),list(Site.Key,Start.Year,SOS,EOS,LGP,Dekad.Season,Tot.Rain)])
# Remove second seasons that are too short
...
if(nrow(Seasonal3)>0){
  ...
  CLIM.Dekad.3<-rbind(CLIM.Dekad1[Site.Key %in% Sites],CLIM.Dekad[!(Site.Key %in% Sites & is.na(Dekad.Season))])
  
  # Update Seasonal3 Stats
  Seasonal3<-unique(CLIM.Dekad.3[!(is.na(Dekad.Season)|is.na(Start.Year)),list(Site.Key,Start.Year,Dekad.Season,SOS,EOS,LGP,Tot.Rain)])
...
# 8) Save data ####
ERA_SOS<-list(Dekadal_SOS=CLIM.Dekad,Seasonal_SOS2=Seasonal2,LTAvg_SOS2=LTAvg_SOS2,Seasonal_SOS3=Seasonal3,LTAvg_SOS3=LTAvg_SOS3)
save(ERA_SOS,file="Large Files/ERA_SOS.RData")



# 9) Explore sites to validate ####
...
View(CLIM.Dekad[Site.Key==SITE])

# 10) Plot seasons to validate ####
# No Seasons
Sites<-CLIM[!Site.Key %in% Seasonal[,unique(Site.Key)],unique(Site.Key)]
Sites<-CLIM[!Site.Key %in% Seasonal3[,unique(Site.Key)],unique(Site.Key)]
