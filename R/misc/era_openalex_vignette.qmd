...
1. Provide a **list** of terms (each block is a vector), or  
2. Provide a **preformatted single string** for that block, containing your boolean operators.

```{r define-terms}
# 1) We define a named list of search blocks. Each block represents a set of terms 
# for a particular aspect of the search. Each list level is vector of character values
# that will be combined with the within_block_operators argument given to the function.
...
# 2) Suppose we want to provide a single preformatted string for 'practice'
#    that includes OR operators, instead of a vector.
...
# Using only a subset of blocks (practice and geo)
test_blocks <- list(
...
We now use the wrapper function to query OpenAlex. To being with we set `download = FALSE` to return hit counts only, rather than downloading the full records. This wis useful for formulating searches quickly withoutn having to download huge amounts of rich bibliographic data from the OA database.
...
# 4. Using Date Filters to Plot Yearly Publication Counts
...
year_hits <- data.frame(year = 1995:2024, hits = NA_integer_)

for (i in seq_along(year_hits$year)) {
  y <- year_hits$year[i]
  # For demonstration, search for "organic fertilizer" in "kenya"
  blocks <- list(
  ...
  year_hits$hits[i] <- res$search_hits
}

plot(
  year_hits$year, year_hits$hits, type="b", pch=19,
  main="Publications in East Africa on 'Organic Fertilizer' (2020-2024)",
...
# 6. Understanding `max_char_limit` and `max_term`
