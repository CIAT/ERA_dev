...
#ESTO AGREGUÉ YO PORQUE EL PIPE ES UNA FUNCIÓN DE DIPLYR QUE HAY QUE INTALAR
library(tidyverse)
...
#Edward
# 4.6) Feed intake 2 #####
feed_intake2<-livestock_metadata$Data.Out[Out.Subind=="Feed Intake",.(B.Code,Country,P.Product,A.Diet.Trees,T.Name,Out.Subind,ED.Intake.Item,ED.Intake.Item.Raw,is_group,is_entire_diet,ED.Mean.T,Out.Unit,ED.Error,ED.Error.Type)]  
feed_intake[is.na(ED.Intake.Item)]
...
datasummary_skim(Tbl_WG)

#CATTLE
Tbl_WGCattle <- filter(WG, 
                       P.Product %in% c ("Cattle"))

Tbl_WGCattleSelect <- Tbl_WGCattle %>% select(B.Code, P.Product, T.Name, A.Level.Name, `Weight Gain_kg`, Out.WG.Start_kg, Out.WG.End)

##Summary
datasummary_skim(Tbl_WGCattle)

#Gráfico
ggplot(Tbl_WGCattle, aes(x=P.Product, y=`Weight Gain_kg`))+
  geom_boxplot()+coord_flip() + ylim(0,2.5)

#GOAT
Tbl_WGGoat <- filter(WG, 
                     P.Product %in% c ("Goat"))

Tbl_WGGoatSelect <- Tbl_WGGoat %>% select(B.Code, P.Product, T.Name, A.Level.Name, `Weight Gain_kg`, Out.WG.Start_kg, Out.WG.End)

#Summary
datasummary_skim(Tbl_WGGoat)

#Gráfico
ggplot(Tbl_WGGoat, aes(x=P.Product, y=`Weight Gain`))+
  geom_boxplot()+coord_flip() + ylim(-100,100)

#SHEEP
Tbl_WGSheep <- filter(WG, 
                      P.Product %in% c ("Sheep"))

Tbl_WGSheepSelect <- Tbl_WGSheep %>% select(B.Code, P.Product, T.Name, A.Level.Name, `Weight Gain_kg`, Out.WG.Start_kg, Out.WG.End)

#Summary
datasummary_skim(Tbl_WGSheep)

#Gráfico
ggplot(Tbl_WGSheep, aes(x=P.Product, y=`Weight Gain`))+
  geom_boxplot()+coord_flip() + ylim(-1000,1000)


#No se puede filtrar por sp arboreas de interés porque no figura la variable
