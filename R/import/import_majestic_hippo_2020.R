...
ZeroFun<-function(Data,Zero.Cols){
  ...
  ## 0.3) Set cores for parallel processing -----
Cores<-detectCores()-1
...
master_codes <- sapply(sheet_names, FUN=function(x){data.table(readxl::read_excel(era_vocab_local, sheet = x))},USE.NAMES=T)
  ...
  ## 1.3) Prepare concepts & harmonization -----
...
VarHarmonization.Traits<-copy(master_codes$var_traits)
#VarHarmonization.Traits<-fread("Concept Scheme/Harmonization/Variety_Traits.csv",header=T,strip.white=F)
VarHarmonization<-copy(master_codes$vars)
#VarHarmonization<-fread("Concept Scheme/Harmonization/Varieties.csv",header=T,strip.white=F)
# Fix bug in names that are decimals
VarHarmonization[,V.Var:=gsub("00000001|00000002","",V.Var)][,V.Var1:=gsub("00000001|00000002","",V.Var1)]
VarHarmonization[,V.Var:=gsub("499999998","5",V.Var)][,V.Var1:=gsub("499999998","5",V.Var1)]
VarHarmonization[,V.Var:=gsub("799999998","8",V.Var)][,V.Var1:=gsub("799999998","8",V.Var1)]

# Read in Corrections
OtherHarmonization<-fread(file.path("data_entry",project,"legacy_files/Other_Fields.csv"),header=T,strip.white=F)
...
SiteHarmonization<-fread(file.path("data_entry",project,"legacy_files/Site_Names.csv"),header=T,strip.white=T,encoding = "Latin-1")
...
Master<-file.path(project_dir,"data_entry",project,"excel_data_extraction_template/V1.14.3 - Majestic Hippo.xlsm")
...
FNames<-unlist(lapply(strsplit(basename(Files)," - "),"[",2))
...
if(length(DuplicateFiles)>0){
  warning("Duplicated Filenames Detected")
...
# 2) Process data ####
  ...
  Site.Out<-data.table(Site.Out)
    ...
    ### 2.2.1) Site.Out: Update Fields From Harmonization Sheet ####
  ...
  setnames(Soil.Out, "Soil.N.Unit...32", "Soil.AN.Unit")
    ...
    ### 2.3.1) Soil.Out: Fix Soils.Out Excel Bug ####
    ...
    ### 2.3.2) Soil.Out: Calculate USDA Soil Texture from Sand, Silt & Clay ####
    ...
    ### 2.3.3) Site.Out: Update Site.Out Soil Texture from USDA Classes ####
    ...
    ### 2.3.4) Soil.Out: Update Fields From Harmonization Sheet ####
  ...
  Var.Out[V.Var=="396034.26799999998",V.Var:="396034.268"]
    ...
    ### 2.6.1) Var.Out: Harmonize Variety Naming and Codes ####
    ...
    VarHarmonization[V.Product=="Potato"]  
    ...
    ### 2.6.2) Var.Out: Update trait labels ####
  ...
  #write.table(Till.Out[!is.na(T.Method.Other)][order(T.Method.Other),unique(T.Method.Other)],"clipboard",row.names = F,sep="\t")
    ...
    ### 2.8.1) Till.Out: Update Fields From Harmonization Sheet ####
  ...
  #Plant.Out[grepl("m3",Plant.Density.Unit)]
    ...
    ### 2.9.1) Plant.Out: Update Fields From Harmonization Sheet ####
  ...
  rm(N)
  ## 2.10) Fertilizer (Fert.Out) =====
  # Fert.Out
  Data<-lapply(XL,"[[","Fert.Out")
  
    ### 2.10.1) Fert.Comp ####
    
    Fert.Comp<-rbindlist(lapply(1:length(Data),FUN=function(i){
      X<-Data[[i]][,-(c(1:36))]
      X$B.Code<-Pub.Out$B.Code[i]
      X<-na.omit(X, cols=c("F.Type...37"))
      X
    }))
    setnames(Fert.Comp, "F.Type...37", "F.Type")
    
    Fert.Comp<-data.table(apply(Fert.Comp,2,FUN = function(X){
      X[X==0]<-NA
      X
    }))
  
    ### 2.10.2) Fert.Method ####
    Fert.Method<-rbindlist(lapply(1:length(Data),FUN=function(i){
      X<-Data[[i]][,-(c(1:21,36:59))]
      X$B.Code<-Pub.Out$B.Code[i]
    ...
    Fert.Method[F.NPK==0,F.NPK:=NA
                  ...
                  ][F.Method==0, F.Method:=NA
                    ][F.Fate==0,F.Fate:=NA
                      ][F.Date==0,F.Date:=NA
                        ][F.Date.Stage==0,F.Date.Stage:=NA
                          ][F.Date.DAP==0,F.Date.DAP:=NA
                            ][F.Date.DAE ==0,F.Date.DAE :=NA
                              ][F.Source ==0,F.Source :=NA
                                ][F.Date.Text==0,F.Date.Text:=NA
                                  ][F.Amount==0,F.Amount:=NA]
    
    # Fert.Method: Remove any parenthesis in names
    Fert.Method[,F.Name:=gsub("[(]","",F.Name)][,F.Name:=gsub("[)]","",F.Name)]
    Fert.Method[,F.Amount:=as.numeric(F.Amount)]
    ...
    fert_master<-unique(master_codes$fert[,list(F.Category,F.Type,F.Category_New,F.Type_New,Fert.Code1,Fert.Code2,Fert.Code3)
    ][,match:=T])
    ...
    # We also need to update the Fert.Comp Table, so merge in the F.Category
    ...
    ][,F.Codes:=paste0(unique(sort(c(Fert.Code1,Fert.Code2,Fert.Code3))),collapse = "-"),by=F.Type.Code2x
    ...
    # Replace values
    Fert.Method[!is.na(F.Type.Code2x),F.Type:=unlist(tstrsplit(F.Type.Code2x,"$$",keep=2,fixed=T))
    ...
    fertilizer_non_match<-Fert.Method[is.na(F.Type.Code2x),list(B.Code=paste(unique(B.Code),collapse="/")),by=F.Type.Codex
    ][,table:="Fert.Method"
    ...
    if(nrow(fertilizer_non_match)>0){
      View(fertilizer_non_match)
    }
    ...
    h_params<-data.table(h_table="Fert.Method",
                         h_field=c("F.Unit","F.Method","F.Source","F.Fate"),
                         h_table_alt=c("Fert.Out",NA,"Fert.Method","Res.Method"),
                         h_field_alt=c("F.Unit",NA,"M.Source","M.Fate"))
    ...
    # Set zeros to NAs (we can't be sure of the difference anyway)
    Fert.Out[F.NO==0,F.NO:=NA
                           ...
                           ][F.O.Unit==0,F.O.Unit:=NA
                             ][F.I.Unit==0,F.I.Unit:=NA]
    ...
    # Update F.Codes
    Fert.Out$F.Codes<-pbapply(data.frame(Fert.Out[,c("F.NI.Code","F.PI.Code","F.KI.Code","F.Urea","F.Compost","F.Manure","F.Biosolid","F.MicroN","F.Biochar")]),1,FUN=function(X){
    ...
    ### 2.10.7) Fert.Out: Add the codes above derived from Fert.Method tab to Fert.Out ####
    ...
    ### 2.10.8) Fert Out: Deal with Urea ####
    ...
    rm(NoFCodes)
    ### 2.10.10) Fert.Out: Add in h10.1 Codes for no Fertilizer use ####
    Fert.Out[is.na(F.Codes) & !F.Level.Name=="Base",F.Codes:="h10.1"
             ][is.na(F.Codes2) & !F.Level.Name=="Base",F.Codes2:="h10.1"]
    
    ### 2.10.11) Fert.Out: To deal with new Ash & Organic (Other) Codes overwrite F.Codes with F.Codes2 ####
    Fert.Out[,F.Codes:=F.Codes2]
    
    ### 2.10.12) Fert.Out/Method: Update Naming Fields ####
    ...
    ### 2.10.13) Fert.Out: Translate Fert Codes in Cols ####
  ...
  ## 2.14) Residues (Res.Out) =====
    ...
    ### 2.14.2) Res.Method - Update Fields From Harmonization Sheet ####
  ...
  WH.Out[,WH.Level.Name:=gsub("[(]","",WH.Level.Name)][,WH.Level.Name:=gsub("[)]","",WH.Level.Name)]
    ...
    ### 2.18.1) WH.Out: Check for potential invalid comparisons that will need ordering practices specifying ####
  ...
  E.Out[,E.Level.Name:=gsub("[(]","",E.Level.Name)][,E.Level.Name:=gsub("[)]","",E.Level.Name)]
    ...
    ### 2.19.1) E.Out: Check for potential invalid comparisons that will need ordering practices specifying ####
  # Count have many ERA practices present for each practice
  ...
  PO.Out[,PO.Level.Name:=gsub("[(]","",PO.Level.Name)][,PO.Level.Name:=gsub("[)]","",PO.Level.Name)]
    ...
    ### 2.20.1) PO.Out: Check for potential invalid comparisons that will need ordering practices specifying ####
  # Count have many ERA practices present for each practice
  ...
  Animals.Diet[D.Item==0,D.Item:=NA]
    ...
    ### 2.21.1) Animals.Diet: Remove any parenthesis in names ####
    ...
    ### 2.21.4) Animals.Out: Validation Checks ####
    Cols<-paste0("P",1:14)
...
MT.Out[,T.Name:=gsub("[(]","",T.Name)
  ...
  MT.Out$T.Codes<-unlist(pblapply(1:nrow(MT.Out),FUN=function(i){
    ...
    if(length(X)>1){
      if(!is.na(MT.Out$T.Codes[i]) & MT.Out$T.Codes[i]=="Aggregated"){
        "Aggregated"
      }else{
  ...
  ## 3.3) MT.Out: Correct Varieties Codes ####
  ...
  ## 3.4) MT.Out: Correct Ridge & Furrow - remove water harvesting code if ridge and furrow is a conventional tillage control ####
  # Add row index
  MT.Out[,N:=1:nrow(MT.Out)]
  ...
  ## 3.5) MT.Out: Add h-codes for Weeding & Herbicide use ####
  ...
  ## 3.9) MT.Out2: Deal with aggregated products  #####
  X<-unlist(pblapply(1:nrow(MT.Out2),FUN=function(i){
    ...
    Y<-lapply(1:length(Y),FUN=function(j){
      ...
      A<-unlist(apply(rbind(A,B),2,FUN=function(X){X[!is.na(X)]}))
...
rm(MT.Out2.ResPrev.vs.ResCode)
  ...
  ## 3.11) MT.Out2: Update final treatment codes (T.Codes) for aggregrated treatments ####
...
# 5) Intercropping (Int.Out)  =====
  ...
  ## 5.1) Int.Out: Remove any parenthesis in names ####
  ...
  ## 5.2) Int.Out: Update Aggregated Treatment Delimiters ####
  # Agg Treat = "..." 
  ...
  ## 5.3) Int.Out: Update Residue Codes ####
  ...
  ## 5.4) Int.Out: Update In.Res column codes for residues that are from Tree List ####
  Int.Out[,N:=1:.N]
  ...
  ## 5.5) Int.Out: Update In.Res column codes for residues that are from Product List #####
  ...
  ## 5.6) Int.Out: Update T.Codes (for updated fertilizer codes & agg practices)  ####
  ...
  ## 5.7) Int.Out: Special rule for when reduced & zero-tillage are both present ####
  ...
  ## 5.9) Int.Out: Update Intercropping Delimiters ####
  ...
  ## 5.11) Int.Out: Residues in System Outcomes ####
...
Rot.Out[R.Phases==0,R.Phases:=NA]
    ...
    ### 6.1.1) Rot.Out: Remove any parenthesis in names ####
    ...
    ### 6.1.2) Rot.Out: Update Delimiters ####
    ...
    ### 6.1.3) Rot.Out: Update R.T.Codes.All ####
  ...
  Rot.Seq[R.Resid.Fate==0,R.Resid.Fate:=NA]
    ...
    ### 6.2.1) Rot.Seq: Add in fallow codes #####
    Rot.Seq[`Rotation Component` == "Natural or Bare Fallow",R.T.Codes:="h24"]
    
    ### 6.2.2) Rot.Seq: Remove any parenthesis in names ####
    ...
    ### 6.2.3) Rot.Seq: Update delimiters ####
    # Add row index
    ...
    ### 6.2.6) Rot.Seq: Summarise Crop Sequence ####
    ...
    ### 6.2.7) Rot.Seq: Update T.Codes - Slow Consider Parallel ####
    ...
    ### 6.2.8) Rot.Seq: Update Residue codes based on previous season ####
    ...
    ### 6.2.9) Rot.Seq: Update residue codes cross-referencing to MASTERCODES sheets ####
  ...
  ## 6.3) Rot.Out: Update Residues Codes from Rot.Seq ####
  # TO DO: NEEDS TO DEAL WITH (REMOVE) YEARS THAT STARTS BEFORE SEQUENCE BEGINNING #####
  ...
  ## 6.4) Rot.Seq: Add Rotation Codes and remove codes for the first year of sequence ####
...
Out.Out[is.na(Out.Depth.Lower),Out.Depth.Upper:=NA] # This line intentionally uses Out.Depth.Lower
  ...
  ## 7.1) Out.Out: Correct old Outcome indicator names ####
...
Out.Out[Out.Subind=="Nitrogen ARE (Unspecificed)",Out.Subind:="Nitrogen Use Efficiency (Unspecificed)"]
  ## 7.2) Out.Out: Update Fields From Harmonization Sheet ####
...
Data.Out[ED.Comparison==0,ED.Comparison:=NA]
  ...
  ## 8.1) Data.Out: Remove any parenthesis in names ####
  ...
  ## 8.2) Data.Out: Add controls for outcomes that are ratios ####
  Data.Out[,N:=1:.N]
  ...
  ## 8.3) Data.Out: Update Fields From Harmonization Sheet ####
  ...
  ## 8.9) Data.Out: Add Products ####
   Data.Out[ED.Product.Simple=="Okra" & ED.Product.Comp =="Fruit (Unspecified)",ED.Product.Comp:="Pods"]
   Data.Out[ED.Product.Simple=="Sugar Cane" & ED.Product.Comp =="Cane",ED.Product.Comp:="Stem/Stalks"]
   Data.Out[ED.Product.Simple=="Zucchini" & ED.Product.Comp =="Fruit (Total Yield)",ED.Product.Comp:="Fruit"]
   
    ### 8.9.1) Data.Out: Add Products: Validation Checks ####
    ...
    ### 8.9.2) Data.Out: Add Products: Join Products to Data.Out ====
      #### 8.9.2.1) Match products in Data.Out to MASTERCODES using product x component code  ####
      ...
      #### 8.9.2.2) For non-matches in 1, match products in Data.Out to MASTERCODES using product only  ####
      N<-which(is.na(X$EU))
      ...
      #### 8.9.2.3) Add in Agroforestry Trees ####
      ...
      #### 8.9.2.4) Repeat 1-3 but for aggregated products (SLOW consider parallel) ####
      ...
      #### 8.9.2.5) Join, order and cbind to Data.Out ####
    ...
    ### 8.9.3) Data.Out: Product Validation ####
    ...
    ### 8.9.4) Data.Out: Recode NA Products ####
  ...
  ## 8.12) Data.Out: Update Crop residues #####
    ### 8.12.1) IN.Residue.Code > T.Residue.Code > R.Residue.Codes.All ####
  ...
  Data.Out[,Final.Residue.Code:=if(!is.na(IN.Residue.Code)){IN.Residue.Code}else{
    ...
    ### 8.12.2) If observation is a component of a rotation treatment update residues from the Rot.Seq table ####
  ...
  ## 8.14) Data.Out: Update Start Year & Season #####
  ...
  ## 8.16) Data.Out: Add Site: Validation ####
  ...
  ## 8.17) Data.Out: Aggregated Sites ####
  ...
  ## 8.18) Data.Out: Times: Validation #####
  ...
  ## 8.19) Data.Out: Add Base Practices #####
  ...
  ## 8.20) Data.Out: Validation: Error Base Codes - Check that only one base treatment is present  #### 
  # 1)Varieties 
  ...
  ## 8.21) Data.Out: Update Structure Fields to reflect Level name rather than "Yes" or "No" ####
  ...
  ## 8.22) Data.Out: Reset N ####
  Data.Out[,N:=NULL][,N:=1:.N]
  ## 8.23) Data.Out: Validation: Check for Character Values in Outcomes and Errors ####
  ...
  ## 8.24) Data.Out: Validation: Check for Errors in Planting and Harvest Dates ####
    if(F){
  ...
  ## 8.25) Data.Out: Add h-codes for monoculture & rainfed ####
  ...
  ## 8.26) Data.Out: Update V.Animal.Practice [.][.][.] codes ####
